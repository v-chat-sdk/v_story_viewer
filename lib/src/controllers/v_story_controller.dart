...
typedef VStoryNavigationCallback = void Function(String? fromStoryId, String toStoryId);
...
class VStoryController extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Future<void> play() async {
    ...
    try {
      ...
      // Start playback
      _updateState(_state.updateStoryState(
  ...
  Future<void> previous() async {
    ...
    try {
      ...
      final previousStory = currentGroup.getPreviousStory(_state.currentStoryId!);
  ...
  void goToStoryByIndex(int index) {
    ...
    if (currentGroup != null && index >= 0 && index < currentGroup.stories.length) {
  ...
  void setMuted(bool muted) {
    _updateState(_state.copyWith(
      storyState: _state.storyState.copyWith(isMuted: muted),
    ));
  ...
  Future<void> _loadVideoController(VVideoStory story) async {
    ...
    if (story.media.networkUrl != null) {
      try {
        ...
        final cachedFile = await _cacheManager.getFile(
          story.media.networkUrl!,
        );
        
        // Use cached file if available
        controller = VideoPlayerController.file(cachedFile);
        debugPrint('✅ Using cached video for story: ${story.id} from ${cachedFile.path}');
      } catch (e) {
        // Fallback to network URL if caching fails
        debugPrint('⚠️ Failed to cache video for story: ${story.id}, using direct network stream: $e');
  ...
  void _startProgressTimer() {
    ...
    final duration = _state.currentStory?.duration ?? const Duration(seconds: 5);
  ...
  void _handleError(dynamic error, [StackTrace? stackTrace]) {
    ...
    _updateState(_state.updateStoryState(VStoryState.error(storyError.userMessage)));
  ...
  ViewStatistics? getViewStatistics() {
    if (!enablePersistence || _state.storyList == null) {
      return null;
    }
  ...
  int getUnviewedCount() {
    if (!enablePersistence || _state.storyList == null) {
      return _state.storyList?.totalUnviewedCount ?? 0;
    }
